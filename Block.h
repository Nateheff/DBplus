...
const uint32_t MAX_BLOCK = 0xfffffffe; //4294967294
...
void Set_Block_Id(Block_Id_Data* block_id, uint32_t block_num){
    block_id->bi_hi = block_num >> 0x10; //Move bits of block num 16 bits right 
...
uint32_t Get_Block_Num(const Block_Id_Data* block_id){
    return((uint32_t)block_id->bi_hi << 0x10)| ((uint32_t)block_id->bi_low);
}